...
Array(10),visit=(source,i)=>{if(isObject(source)){if(0<=stack.indexOf(source))return;if(!("toJSON"in source)){stack[i]=source;const target=isArray(source)?[]:{};forEach(source,(value,key)=>{value=visit(value,i+1);!isUndefined(value)&&(target[key]=value)});stack[i]=void 0;return target}}return source};return visit(obj,0)}}}
